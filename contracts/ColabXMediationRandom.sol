...
contract CollabXMediationRandom is ControllerOwnable, Initializable {
  ...
  // call the controller contract as a ICollabXMediationRequester
  function mediationAcceptResult(
    ...
    ICollabXMediationRequester(getControllerAddress()).mediationAcceptResult(dealId);
  }
  ...
  function mediationRejectResult(
    ...
    require(mediators[dealId] == tx.origin, "tx.origin must be the mediator");
    // call the controller contract
    ICollabXMediationRequester(getControllerAddress()).mediationRejectResult(dealId);
  }
}
