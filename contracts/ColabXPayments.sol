...
import "./ICollabXToken.sol";
import "./ControllerOwnable.sol";
import "./ICollabXPayments.sol";
...
contract CollabXPayments is ControllerOwnable, Initializable {

  /**
   * Types
   */

  // the address of the CollabXToken.sol contract
  address private tokenAddress;
  ICollabXToken private tokenContract;
  ...
  // the CollabXToken.sol we are calling payinEscrow and payoutEscrow on
  function setTokenAddress(address _tokenAddress) public onlyOwner {
    require(_tokenAddress != address(0), "CollabXPayments: Token address must be defined");
    require(canChangeTokenAddress, "CollabXToken: canChangeTokenAddress is disabled");
    tokenAddress = _tokenAddress;
    tokenContract = ICollabXToken(_tokenAddress);
  }
  ...
  function agreeResourceProvider(
    ...
    require(tx.origin == resourceProvider, "CollabXPayments: Can only be called by the RP");
    _payEscrow(
      dealId,
      timeoutCollateral,
  ...
  function agreeJobCreator(
    ...
    require(tx.origin == jobCreator, "CollabXPayments: Can only be called by the JC");
    _payEscrow(
      dealId,
      paymentCollateral,
  ...
  function timeoutMediateResult(
    ...
    require(tx.origin == resourceProvider || tx.origin == jobCreator, "CollabXPayments: Can only be called by the RP or JC");
    // the refund of the results collateral to the RP
    _refundEscrow(
      dealId,
      resourceProvider,
  ...
  function _payEscrow(
    ...
    require(tokenContract.balanceOf(tx.origin) >= amount, "CollabXPayments: Insufficient balance");
    ...
    require(success, "CollabXPayments: Pay escrow failed");
  ...
  function _refundEscrow(
    ...
    require(success, "CollabXPayments: Refund escrow failed");
  ...
  function _payOut(
    ...
    require(success, "CollabXPayments: Pay job failed");
  ...
  function _slashEscrow(
    ...
    require(success, "CollabXPayments: Slash escrow failed");
