...
contract ColabXUsers is Ownable, Initializable {
    ...
    function addUserToList(SharedStructs.ServiceType serviceType) public {
        ...
        require(
            _doesUserHaveRole(serviceType, tx.origin),
            "User must have that role"
        );
        usersByType[serviceType].push(tx.origin);
    }

    function removeUserFromList(SharedStructs.ServiceType serviceType) public {
        require(users[tx.origin].userAddress != address(0), "User must exist");
